...
function calendrier($m_donne,$a_donne, $id_medecin, $id_patient, $heure_rdv){
    ...
    echo "<table border=1 bordercolor=\"#FFFFFF\"><tr><td class=\"flechegauche\">"
        .mois_precedent($m,$mois[$m],$a)
        ."</td><td class=\"nom_mois\" colspan=\"5\">$mois[$m] $a</td><td class=\"flechedroite\">"
        .mois_suivant($m,$a)
    ...
    for($i=1;$i<40;$i++){        
        ...
        $joursmoisavant++;
            ...
            if ($jour == date("d") && $m == date("n") && $a == date("Y")) {
                ...
                echo "<td class=\"aujourdhui\" onclick=\"openModal('$jour-$m-$a')\">$jour</td>";
            } elseif ($jour < date("d") && $m == date("n") && $a == date("Y")) {
                // Si le jour est avant la date actuelle, appliquer la classe 'precedent'
